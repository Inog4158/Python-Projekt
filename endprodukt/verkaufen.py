...
class Verkaufen(Button):
  def verkaufen (self):
    class MyButton (Button):
      def aktion (self):
        artnr = eingabe1.get()
        f = open(r'C:\Users\inog\OneDrive\Desktop\Development\Python\Python Projekt\.endprodukt\dateien\sortiment','r') 
        liste = f.readlines() 
        f.close() 
        geloescht = False 
        for i in range(len(liste)):
          if liste[i] == artnr+"\n":
            f = open(r'C:\Users\inog\OneDrive\Desktop\Development\Python\Python Projekt\.endprodukt\dateien\sortiment','w')
            liste = liste[:i] + liste[i+6:]
            for linie in liste:
              f.write(linie)      # eventuell fehler
              geloescht = True 
            break
        f.close() 
        fenster.destroy() 
        if geloescht:
          ...
          fenster2.title("Verkauft!")
          rahmen2 = Frame(fenster2, relief="ridge", borderwidth=5) 
          rahmen2.pack(fill="both", expand = 1)
          label2 = Label(rahmen2, text="Artikel aus dem Sortiment entfernt!") 
    ...
    label = Label(rahmen, text = "Welchen Artikel möchten Sie verkaufen?")
